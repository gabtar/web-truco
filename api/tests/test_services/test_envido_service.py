...
def test_can_chant_envido_during_first_round(
    ...
    envido_manager = EnvidoManager(hands=fake_hands_repository, games=fake_games_repository)
...
def test_cannot_chant_envido_if_first_round_has_finished(
    ...
    # Play 2 cards to complete 1st round
...
def test_cannot_chant_envido_if_not_player_turn(
    ...
    with pytest.raises(GameException) as excep:
        envido_manager.chant_envido(hand_id=hand.id, player_id=hand.player_dealer, level=EnvidoLevels.FALTA_ENVIDO)
...
def test_if_player2_envido_score_is_higher_he_wins_the_envido(
    ...
    envido_manager.chant_envido(hand_id=hand.id, player_id=hand.chant_turn, level=EnvidoLevels.REAL_ENVIDO)
    envido_manager.accept_envido(hand_id=hand.id, player_id=hand.chant_turn)

    # Primero juega las cartas el jugador que es mano -> Canta 29
    envido_manager.play_envido(
            hand_id=hand.id,
            player_id=hand.chant_turn,
            # 29
    ...
    # 32 > 29 gana el segundo jugador que canta
    assert hand.envido.winner == hand.player_dealer
...
def test_if_both_players_have_equal_score_the_hand_player_wins(
    ...
    hand: Hand = fake_hands_repository.get_by_id(id='game1')
    envido_manager = EnvidoManager(hands=fake_hands_repository, games=fake_games_repository)
    player1 = hand.player_hand
    player2 = hand.player_dealer
